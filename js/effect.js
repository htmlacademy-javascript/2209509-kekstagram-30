...
const imageElememnt = modalElement.querySelector('.img-upload__preview img');
...
const setImageStyle = () => {
  if(isDefault()) {
    imageElememnt.style.filter = null;
    return;
  }

  const { value } = effectLevelElement;
  const { style, unit } = effectFilter(chosenEffect);
  imageElememnt.style.filter = `${style}(${value}${unit})`;
};
...
const createSlider = ({ min, max, step }) => {
  noUiSlider.createSlider.create(sliderElement, {
    range: {min, max},
    step,
    start: max,
    connect: 'lower',
